...
class ErrorCorrectionSeq2SeqDataset(Dataset):
    ...
    def prepare_src(self, source, instance):
        ...
        if instance["mutation_type"] is not None and self.mutation_src:
            claims = []
            for x in ['title:'+e for e in instance['evidence'].split('title:')][1:]:
                claims.append("claim: "+source+" "+"mutation: "+instance["mutation_type"]+" "+"evidence: "+x)
            return claims
        else:
            claims = []
            for x in ['title:' + e for e in instance['evidence'].split('title:')][1:]:
                claims.append(
                    "claim: " + source + " " + "evidence: " + x)
            return claims

    def prepare_tgt(self, target, instance):
        if instance["mutation_type"] is not None and self.mutation_tgt:
            return (
                ["mutation: " + instance["mutation_type"] + " " + "correction: " + target + " </s>"]
            )

        return ["correction: " + target + " </s>"]
    ...
    def __getitem__(self, index) -> Dict[str, torch.Tensor]:
        ...
        if len(source_inputs["input_ids"].size()) > 2:
            source_ids = source_inputs["input_ids"].squeeze()
        if len(target_inputs["input_ids"].size()) > 2:
            target_ids = target_inputs["input_ids"].squeeze()
        if len(source_inputs["attention_mask"].size()) > 2:
            src_mask = source_inputs["attention_mask"].squeeze()
        if len(original_ids["input_ids"].size()) > 2:
